...
const { isSnapshot, genAuthSig } = require('bfx-api-node-util')
const debug = require('debug')('bfx:api:ws1')
...
class WSv1 extends EventEmitter {
   ...
   * @param {string} [opts.url='wss://api.bitfinex.com/ws'] - connection url
   */
  ...
  /**
   * @param {string} msgJSON - message as JSON
  ...
  _handleChannel (msg) {
    ...
    if (msg[0] === 'hb') {
      debug(`received heartbeat in ${event.channel}`)
      return
    }
   ...
   * string.
   *
   * @param {object|Array} msg - message
   */
  send (msg) {
    debug('Sending %j', msg)
   ...
   * @param {string} [prec='P0'] - price aggregation level (P0, P1, P2, P3)
   * @param {string} [len='25'] - number of price points. 25 or 100.
